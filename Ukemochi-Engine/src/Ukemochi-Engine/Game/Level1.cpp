...
#include "Ukemochi-Engine/Logic/DataSyncSystem.h"
...
namespace Ukemochi
{
    ...
    void Level1_Initialize() //Initialize the game at the start of Level1
    {
        ...
        level_background.AddComponent(Transform{
            Mtx44{},
            Vec2{
                ECS::GetInstance().GetSystem<Renderer>()->screen_width * 0.5f,
    ...
    void Level1_Update() //Level1 game runtime
    {
        ...
        if (Ukemochi::Input::IsKeyTriggered(GLFW_KEY_L)) //test run_time cloning
        {
            ...
            clone.GetComponent<Transform>().position = Vec2{
                clone.GetComponent<Transform>().position.x + 5.f, clone.GetComponent<Transform>().position.y + 1.f
            };
        }
