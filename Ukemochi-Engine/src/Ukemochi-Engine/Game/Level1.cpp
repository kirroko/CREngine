...
namespace Ukemochi
{
	...
	void Level1_Initialize()//Initialize the game at the start of Level1
	{
		...
		player_rb.position_curr.x = renderer_system.screen_width * 0.5f;
		player_rb.position_curr.y = renderer_system.screen_height * 0.5f;
	}
	...
	void Level1_Update()//Level1 game runtime
	{
		...
		Physics::UpdatePhysics(player_rb, player_box_collider);

		// Check collisions
		float tLast;
		if (Collision::CollisionIntersection_BoxBox(player_box_collider, player_rb.velocity, player_box_collider, player_rb.velocity, tLast))
		{
			// do collision response
			//player_rb.position = player_rb.initial_position;
			//std::cout << "box box collided\n";
		}
		//if (Collision::CollisionIntersection_BoxScreen(player_box_collider, renderer_system.screen_width, renderer_system.screen_height))
		//{
	...
	void Level1_Draw()//rendering of the game for Level1
	{
		...
		renderer_system.drawBox(player_rb.position_curr.x, player_rb.position_curr.y, player_rb.scale.x, player_rb.scale.y, "../Assets/Textures/container.jpg");
