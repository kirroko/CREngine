...
void Renderer::render()
{
	...
	for (auto& entity : m_Entities)
	{
		...
		if (ECS::GetInstance().HasComponent<Animation>(entity))
		{
			...
			batchRenderer->drawSprite(glm::vec3(renderPos, 0.f), finalScale, glm::vec3(1.0f, 1.0f, 1.0f), mappedTextureUnit, uvCoordinates, glm::radians(transform.rotation), 0);
		}
		else
		{
			...
			batchRenderer->drawSprite(glm::vec3(transform.position.x, transform.position.y, 0.f), glm::vec2(transform.scale.x, transform.scale.y), glm::vec3(1.0f, 1.0f, 1.0f), mappedTextureUnit, uvCoordinates, glm::radians(transform.rotation), 0);
		}
	}
